...
def download_dynet_model(
    ...
    if resource_dir is not None and os.path.exists(
        os.path.join(resource_dir, f"{lang_name}")
    ):
        if confirm_if_exists:
            print("")
            print(
                f'The model directory already exists at "{resource_dir}/{lang_name}". Do you want to download the models again? [y/N]'
            )
...
def download(
    ...
    if download_language in languages:
        download_dynet_model(
            download_language,
            resource_dir=resource_dir,
            confirm_if_exists=confirm_if_exists,
            force=force,
            version=version,
        )
    else:
        raise ValueError(
            f'The language "{download_language}" is not currently supported by this function. Please try again with other languages.'
        )
